'''
The key codes used here were copied from OIS library header.
...
class InputListener:
    ...
    def add_input_event(self, name):
        self.events[name] = InputEvent(name)
        
    def __init_events(self):
        self.add_input_event("exit_rig_editor")
        
    def add_event_mapping(self, key_code, event):
        self.key_event_map[key_code] = event
        
    def __init_event_mappings(self):
        self.add_event_mapping(KeyCodes.ESCAPE, self.events["exit_rig_editor"])
    ...
    def key_pressed_callback(self, key_code, key_value):
        event = self.key_event_map[key_code]
