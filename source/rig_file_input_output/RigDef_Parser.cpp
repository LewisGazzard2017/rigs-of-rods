...
{
	/* Push defaults */
	m_ror_default_inertia = std::shared_ptr<DefaultInertia>(new DefaultInertia);
...
void Parser::ParseTractionControl(Ogre::String const & line)
{
	...
	m_current_module->traction_control = std::shared_ptr<TractionControl>( new TractionControl() );
...
void Parser::ParseDirectiveSetManagedMaterialsOptions(Ogre::String const & line)
{
	...
	if (! std::regex_match(input, std::regex("0|1", std::regex::extended)))
	{
		AddMessage(
			line, 
...
void Parser::ParseHook(Ogre::String const & line)
{
	...
	{
		std::smatch results;
		if (! std::regex_search(*iter, results, Regexes::HOOKS_OPTIONS))
		{
...
void Parser::ParseFusedrag(Ogre::String const & line)
{
	...
	if (results[3].matched)
	{
		...
		m_current_module->fusedrag = std::shared_ptr<Fusedrag>( new Fusedrag(fusedrag) );
	}
	else if (results[6].matched)
	{
...
void Parser::ParseFlexbody(Ogre::String const & line)
{
	...
	else if (m_current_subsection == File::SUBSECTION__FLEXBODIES__FORSET_LINE)
	{
		...
		{
			std::smatch results;
			if (! std::regex_search(*iter, results, Regexes::FORSET_ELEMENT))
			{
...
{
	std::smatch results;
	if (std::regex_search(node_id_str, results, Regexes::NODE_NUMBER))
	{
	...
	{
		if (std::regex_search(node_id_str, results, Regexes::NODE_NAME))
		{
...
void Parser::ParseDirectiveSetInertiaDefaults(Ogre::String const & line)
{
	...
	{
		// Create
		m_user_default_inertia = std::shared_ptr<DefaultInertia>(new DefaultInertia(*m_user_default_inertia.get()));
...
void Parser::_ParseSectionsNodesNodes2(Ogre::String const & line)
{
	// Parse line
	std::smatch results;
	if (! std::regex_search(line, results, Regexes::SECTION_NODES_N2))
	{
		AddMessage(line, Message::TYPE_ERROR, "Invalid line, ignoring...");
		return;
	}
...
bool Parser::_ParseOptionalInertia(OptionalInertia & inertia, std::smatch & results, unsigned int start_index)
{
...
{
	std::smatch results;
	if (std::regex_search(line, results, Regexes::IDENTIFY_KEYWORD))
	{
