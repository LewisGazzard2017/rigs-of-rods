...
void Collisions::SetupLandUse(std::string const & landuse_cfg_filename)
{
#ifdef USE_PAGED
    try
    {
        RoR::LandUseParser parser;
        parser.LoadLandUseCfg(landuse_cfg_filename);
        std::shared_ptr<RoR::LandUseDef> def = parser.GetLandUseDef();

        // Apply map-specific settings
        for (std::string const & gm_filename: def->groundmodel_files)
        {
            this->loadGroundModelsConfigFile(gm_filename);
        }

        if (!def->default_groundmodel.empty())
        {
            m_land_use.outside = this->getGroundModelByString(def->default_groundmodel);
        }

        // Prepare a color -> groundmodel conversion table
        std::map<size_t, ground_model_t*> color2model;
        for (RoR::LandUseEntry& entry: def->mappings)
        {
            color2model.insert(std::make_pair(entry.rgba_color, this->getGroundModelByString(entry.groundmodel_name)));
        }
        ...
        Forests::ColorMap* color_map = Forests::ColorMap::load(def->texture_file, Forests::CHANNEL_COLOR);
