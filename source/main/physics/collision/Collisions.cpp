...
void Collisions::SetupLandUse(std::string const & landuse_cfg_filename)
{
    ...
    try
    {
        ...
        {
            m_ground_models.LoadGroundModels(gm_filename);
        }

        if (!def->default_groundmodel.empty())
        {
            m_land_use.gm_outside = m_ground_models.GetGroundModel(def->default_groundmodel.c_str());
        }
        ...
        for (RoR::LandUseEntry& entry: def->mappings)
        {
            color2model.insert(std::make_pair(
                entry.rgba_color,
                m_ground_models.GetGroundModel(entry.groundmodel_name.c_str())));
        }
