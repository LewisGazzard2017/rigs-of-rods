...
void AsSetupHelper::RegisterGlobalFn(const char *decl, const AngelScript::asSFuncPtr &funcPointer, AngelScript::asDWORD callConv)
{
    ...
    if (result < 0)
    {
        this->SetError(result) << "RegisterGlobalFunction("<<decl<<") failed, message: " << AsRetCodeToString(result);
    }
}

void AsSetupHelper::ResetError()
{
    m_last_err_code = 0;
    m_last_err_msg.str(""); // Reset
}

std::stringstream& AsSetupHelper::SetError(int err)
{
    m_last_err_code = err;
    ...
    m_typeid(0)
{
    ...
    {
        m_setup_helper->SetError(typeid_or_error) << "RegisterObjectType("<<m_object_name<<") failed, return code: " << typeid_or_error;
    }
    else
    {
...
void AsObjectRegProxy::AddBehavior(AngelScript::asEBehaviours behaviour, const char *declaration, const AngelScript::asSFuncPtr &funcPointer, AngelScript::asDWORD callConv)
{
    ...
    if (id_or_error < 0)
    {
        m_setup_helper->SetError(id_or_error) << "RegisterObjectBehaviour("<<m_object_name<<", "<<declaration
            <<") failed, message: " << AsRegPropertyErrorToString(id_or_error);
    }
}
...
const char* AsRegPropertyErrorToString(int ret_code)
{
    switch (ret_code)
    {
    case AngelScript::asWRONG_CONFIG_GROUP:  return "asWRONG_CONFIG_GROUP: The object type was registered in a different configuration group.";
    case AngelScript::asINVALID_OBJECT:      return "asINVALID_OBJECT: The obj does not specify an object type.";
    case AngelScript::asINVALID_TYPE:        return "asINVALID_TYPE: The obj parameter has invalid syntax.";
    case AngelScript::asINVALID_DECLARATION: return "asINVALID_DECLARATION: The declaration is invalid.";
    case AngelScript::asNAME_TAKEN:          return "asNAME_TAKEN: The name conflicts with other members.";
...
const char* AsRegMethodErrorToString(int ret_code)
{
    ...
    {
    case AngelScript::asWRONG_CONFIG_GROUP:  return "asWRONG_CONFIG_GROUP: The object type was registered in a different configuration group.";
    case AngelScript::asNOT_SUPPORTED:       return "asNOT_SUPPORTED: The calling convention is not supported.";
    case AngelScript::asINVALID_TYPE:        return "asINVALID_TYPE: The obj parameter is not a valid object name.";
    case AngelScript::asINVALID_DECLARATION: return "asINVALID_DECLARATION: The declaration is invalid.";
    case AngelScript::asNAME_TAKEN:          return "asNAME_TAKEN: The name conflicts with other members.";
    case AngelScript::asWRONG_CALLING_CONV:  return "asWRONG_CALLING_CONV: The function's calling convention isn't compatible with callConv.";
    default:                                 return "~Unknown error code~";
    }
}
