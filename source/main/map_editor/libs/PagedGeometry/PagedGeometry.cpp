...
void PagedGeometry::setPageSize(Real size)
{
	if (!managerList.empty())
		OGRE_EXCEPT(Ogre::Exception::ExceptionCodes(0), "PagedGeometry::setPageSize() cannot be called after detail levels have been added. Call removeDetailLevels() first.", "PagedGeometry::setPageSize()");
...
{
	if (!managerList.empty())
		OGRE_EXCEPT(Ogre::Exception::ExceptionCodes(0), "PagedGeometry::setInfinite() cannot be called after detail levels have been added. Call removeDetailLevels() first.", "PagedGeometry::setInfinite()");
...
{
	if (!managerList.empty())
		OGRE_EXCEPT(Ogre::Exception::ExceptionCodes(0), "PagedGeometry::setBounds() cannot be called after detail levels have been added. Call removeDetailLevels() first.", "PagedGeometry::setBounds()");
...
void PagedGeometry::_addDetailLevel(GeometryPageManager *mgr, Real maxRange, Real transitionLength)
{
	...
	if (maxRange <= minRange){
		OGRE_EXCEPT(Ogre::Exception::ExceptionCodes(1), "Closer detail levels must be added before farther ones", "PagedGeometry::addDetailLevel()");
	}
