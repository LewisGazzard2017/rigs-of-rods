...
DensityMap::DensityMap(TexturePtr map, MapChannel channel)
{
	...
	if (channel == CHANNEL_COLOR){
		...
		switch (channel){
			...
			default: OGRE_EXCEPT(Ogre::Exception::ExceptionCodes(0), "Invalid channel", "GrassLayer::setDensityMap()"); break;
		}
...
ColorMap::ColorMap(TexturePtr map, MapChannel channel)
{
	...
	switch (format){
		...
		case VET_COLOUR_ABGR:
			...
			break;
		default:
			OGRE_EXCEPT(Ogre::Exception::ExceptionCodes(0), "Unknown RenderSystem color format", "GrassLayer::setColorMap()");
			break;
	}
	...
	if (channel == CHANNEL_COLOR){
		...
		switch (channel){
			...
			case CHANNEL_BLUE: channelOffset = 1; break;
			case CHANNEL_ALPHA: channelOffset = 0; break;
            default: OGRE_EXCEPT(Ogre::Exception::ExceptionCodes(0), "Invalid channel", "ColorMap::ColorMap()"); break;
		}
