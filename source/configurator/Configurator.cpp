...
public:
	KeyTestDialog(MyDialog *_dlg) : wxDialog(NULL, wxID_ANY, wxString(), wxDefaultPosition, wxSize(500,600), wxSTAY_ON_TOP|wxDOUBLE_BORDER|wxCLOSE_BOX), dlg(_dlg)
	{
		...
		{
            wxLogStatus("Unable to open inputs!");
		}
...
MyDialog::MyDialog(const wxString& title, MyApp *_app) : wxDialog(NULL, wxID_ANY, title,  wxPoint(100, 100), wxSize(500, 580), wxDEFAULT_DIALOG_STYLE|wxRESIZE_BORDER), openCLAvailable(0)
{
	...
	wxLogStatus("InputEngine starting");

	wxFileName tfn=wxFileName(app->UserPath, wxEmptyString);
	tfn.AppendDir(_T("config"));
	size_t hWnd = (size_t)this->GetHandle();
	wxLogStatus("Searching input.map in "+tfn.GetPath());
	...
	if(!INPUTENGINE.setup(hwnd_str, false, false, 0))
	{
		wxLogError("Unable to setup inputengine!");
	}
    else
	{
		if (!INPUTENGINE.loadMapping(path+"input.map"))
		{
			wxLogError("Unable to open default input map!");
		}
	}
	wxLogStatus("InputEngine started");
	kd=0;
...
//	wxBitmap *bitmap=new wxBitmap("data\\config.png", wxBITMAP_TYPE_BMP);
    ...
    */
...
#else
    {
        // Intentionally shadowing the controlsPanel variable - testing
        /*OHLwxPanel *controlsPanel=new wxPanel(ctbook, -1);
	    ctbook->AddPage(controlsPanel, _("Input mapping"), false);
        */
	...
	sizer_controls->Add(cTree, /*2,*/ 0, wxGROW);
	...
	*/
    ...
    wxPanel *ffPanel = nullptr;
    {
        auto ctbook = nbook; // test shadowing
	ffPanel=new wxPanel(ctbook, -1);
